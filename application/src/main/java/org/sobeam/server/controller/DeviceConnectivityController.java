...
import org.sobeam.server.dao.device.DeviceConnectivityService;
...
import static org.sobeam.server.controller.ControllerConstants.PROTOCOL;
import static org.sobeam.server.controller.ControllerConstants.PROTOCOL_PARAM_DESCRIPTION;
...
import static org.sobeam.server.dao.util.DeviceConnectivityUtil.DOCKER_COMPOSE_YML;
...
public class DeviceConnectivityController extends BaseController {
    ...
    public JsonNode getDevicePublishTelemetryCommands(@Parameter(description = DEVICE_ID_PARAM_DESCRIPTION)
                                                      @PathVariable(DEVICE_ID) String strDeviceId, HttpServletRequest request) throws SobeamException, URISyntaxException {
        checkParameter(DEVICE_ID, strDeviceId);
        DeviceId deviceId = new DeviceId(toUUID(strDeviceId));
        Device device = checkDeviceId(deviceId, org.sobeam.server.service.security.permission.Operation.READ_CREDENTIALS);
    ...
    public ResponseEntity<org.springframework.core.io.Resource> downloadServerCertificate(@Parameter(description = PROTOCOL_PARAM_DESCRIPTION)
                                                                                          @PathVariable(PROTOCOL) String protocol) throws SobeamException, IOException {
    ...
    public ResponseEntity<org.springframework.core.io.Resource> downloadGatewayDockerCompose(@Parameter(description = DEVICE_ID_PARAM_DESCRIPTION)
                                                                                             @PathVariable(DEVICE_ID) String strDeviceId, HttpServletRequest request) throws SobeamException, URISyntaxException, IOException {
        checkParameter(DEVICE_ID, strDeviceId);
        DeviceId deviceId = new DeviceId(toUUID(strDeviceId));
        Device device = checkDeviceId(deviceId, Operation.READ_CREDENTIALS);

        if (!checkIsGateway(device)) {
            throw new SobeamException("The device must be a gateway!", SobeamErrorCode.BAD_REQUEST_PARAMS);
        }
