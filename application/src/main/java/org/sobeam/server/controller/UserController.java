...
import org.sobeam.server.common.data.UserEmailInfo;
...
import org.sobeam.server.common.data.mobile.MobileSessionInfo;
...
import org.sobeam.server.service.security.model.UserPrincipal;
...
import static org.sobeam.server.controller.ControllerConstants.ALARM_ID_PARAM_DESCRIPTION;
...
import static org.sobeam.server.controller.ControllerConstants.DASHBOARD_ID_PARAM_DESCRIPTION;
import static org.sobeam.server.controller.ControllerConstants.DEFAULT_DASHBOARD;
import static org.sobeam.server.controller.ControllerConstants.HOME_DASHBOARD;
...
import static org.sobeam.server.controller.ControllerConstants.USER_ID_PARAM_DESCRIPTION;
import static org.sobeam.server.controller.ControllerConstants.USER_TEXT_SEARCH_DESCRIPTION;
import static org.sobeam.server.controller.ControllerConstants.UUID_WIKI_LINK;
import static org.sobeam.server.dao.entity.BaseEntityService.NULL_CUSTOMER_ID;
...
public class UserController extends BaseController {
    ...
    public User getUserById(
            ...
            @PathVariable(USER_ID) String strUserId) throws SobeamException {
    ...
    public User saveUser(
            ...
            @RequestParam(required = false, defaultValue = "true") boolean sendActivationMail, HttpServletRequest request) throws SobeamException {
    ...
    public void deleteUser(
        ...
        if (user.getAuthority() == Authority.SYS_ADMIN && getCurrentUser().getId().equals(userId)) {
            throw new SobeamException("Sysadmin is not allowed to delete himself", SobeamErrorCode.PERMISSION_DENIED);
        }
    ...
    public void setUserCredentialsEnabled(
            ...
            @RequestParam(required = false, defaultValue = "true") boolean userCredentialsEnabled) throws SobeamException {
        checkParameter(USER_ID, strUserId);
        UserId userId = new UserId(toUUID(strUserId));
    ...
    public JsonNode saveUserSettings(@RequestBody JsonNode settings) throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
    ...
    public void putUserSettings(@RequestBody JsonNode settings) throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
    ...
    public JsonNode getUserSettings() throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
    ...
    public void deleteUserSettings(@Parameter(description = PATHS)
                                   @PathVariable(PATHS) String paths) throws SobeamException {
    ...
    public void putUserSettings(@Parameter(description = "Settings type, case insensitive, one of: \"general\", \"quick_links\", \"doc_links\" or \"dashboards\".")
                                @PathVariable("type") String strType, @RequestBody JsonNode settings) throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
    ...
    public JsonNode getUserSettings(@Parameter(description = "Settings type, case insensitive, one of: \"general\", \"quick_links\", \"doc_links\" or \"dashboards\".")
                                    @PathVariable("type") String strType) throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
        UserSettingsType type = checkEnumParameter("Settings type", strType, UserSettingsType::valueOf);
        checkNotReserved(strType, type);
    ...
    public UserDashboardsInfo getUserDashboardsInfo() throws SobeamException {
        SecurityUser currentUser = getCurrentUser();
    ...
    public UserDashboardsInfo reportUserDashboardAction(
            ...
            @PathVariable("action") String strAction) throws SobeamException {
    ...
    private void checkNotReserved(String strType, UserSettingsType type) throws SobeamException {
        if (type.isReserved()) {
            throw new SobeamException("Settings with type: " + strType + " are reserved for internal use!", SobeamErrorCode.BAD_REQUEST_PARAMS);
        }
    }
