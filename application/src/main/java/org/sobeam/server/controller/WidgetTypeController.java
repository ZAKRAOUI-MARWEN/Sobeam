...
import org.sobeam.server.service.entitiy.widgets.type.TbWidgetTypeService;
...
import static org.sobeam.server.controller.ControllerConstants.WIDGET_TYPE_ID_PARAM_DESCRIPTION;
import static org.sobeam.server.controller.ControllerConstants.WIDGET_TYPE_TEXT_SEARCH_DESCRIPTION;
...
public class WidgetTypeController extends AutoCommitController {
    ...
    public WidgetTypeInfo getWidgetTypeInfoById(
            @Parameter(description = WIDGET_TYPE_ID_PARAM_DESCRIPTION, required = true)
            @PathVariable("widgetTypeId") String strWidgetTypeId) throws SobeamException {
        checkParameter("widgetTypeId", strWidgetTypeId);
        WidgetTypeId widgetTypeId = new WidgetTypeId(toUUID(strWidgetTypeId));
    ...
    public PageData<WidgetTypeInfo> getWidgetTypes(
            ...
            @RequestParam(required = false) String[] widgetTypeList) throws SobeamException {
    ...
    public List<WidgetType> getBundleWidgetTypesByBundleAlias(
            ...
            @RequestParam String bundleAlias) throws SobeamException {
        TenantId tenantId;
        if (isSystem) {
    ...
    public List<WidgetType> getBundleWidgetTypes(
            ...
            @RequestParam("widgetsBundleId") String strWidgetsBundleId) throws SobeamException {
    ...
    public WidgetType getWidgetTypeByBundleAliasAndTypeAlias(
            ...
            @RequestParam String alias) throws SobeamException {
        TenantId tenantId;
        if (isSystem) {
    ...
    public WidgetType getWidgetType(
            ...
            @RequestParam String fqn) throws SobeamException {
        ...
        if (parts.length < 2 || (!scopeQualifier.equals("system") && !scopeQualifier.equals("tenant"))) {
            throw new SobeamException("Invalid fqn!", SobeamErrorCode.BAD_REQUEST_PARAMS);
        }
        TenantId tenantId;
