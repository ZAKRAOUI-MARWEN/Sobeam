 ...
 */
package org.sobeam.server.install;
...
import org.sobeam.server.service.install.DatabaseEntitiesUpgradeService;
import org.sobeam.server.service.install.EntityDatabaseSchemaService;
import org.sobeam.server.service.install.InstallScripts;
import org.sobeam.server.service.install.NoSqlKeyspaceService;
import org.sobeam.server.service.install.SystemDataLoaderService;
import org.sobeam.server.service.install.TsDatabaseSchemaService;
import org.sobeam.server.service.install.TsLatestDatabaseSchemaService;
import org.sobeam.server.service.install.migrate.TsLatestMigrateService;
import org.sobeam.server.service.install.update.CacheCleanupService;
import org.sobeam.server.service.install.update.DataUpdateService;

import static org.sobeam.server.service.install.update.DefaultDataUpdateService.getEnv;

@Service
@Profile("install")
@Slf4j
public class SobeamInstallService {
    ...
    public void performInstall() {
        try {
            if (isUpgrade) {
                log.info("Starting SoBeam Upgrade from version {} ...", upgradeFromVersion);

                cacheCleanupService.clearCache(upgradeFromVersion);

                if ("cassandra-latest-to-postgres".equals(upgradeFromVersion)) {
                    log.info("Migrating SoBeam latest timeseries data from cassandra to SQL database ...");
                    ...
                    switch (upgradeFromVersion) {
                        case "3.5.0":
                            log.info("Upgrading SoBeam from version 3.5.0 to 3.5.1 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.5.0");
                        case "3.5.1":
                            log.info("Upgrading SoBeam from version 3.5.1 to 3.6.0 ...");
                        ...
                        case "3.6.0":
                            log.info("Upgrading SoBeam from version 3.6.0 to 3.6.1 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.6.0");
                            dataUpdateService.updateData("3.6.0");
                        case "3.6.1":
                            log.info("Upgrading SoBeam from version 3.6.1 to 3.6.2 ...");
                        ...
                        case "3.6.2":
                            log.info("Upgrading SoBeam from version 3.6.2 to 3.6.3 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.6.2");
                            systemDataLoaderService.updateDefaultNotificationConfigs(true);
                        case "3.6.3":
                            log.info("Upgrading SoBeam from version 3.6.3 to 3.6.4 ...");
                            databaseEntitiesUpgradeService.upgradeDatabase("3.6.3");
                        case "3.6.4":
                            log.info("Upgrading SoBeam from version 3.6.4 to 3.7.0 ...");
                            ...
                            break;
                        default:
                            throw new RuntimeException("Unable to upgrade SoBeam, unsupported fromVersion: " + upgradeFromVersion);
                    }
                ...
                log.info("Starting SoBeam Installation...");
...
//                systemDataLoaderService.loadSystemRules();
            ...
            log.error("Unexpected error during SoBeam installation!", e);
            throw new SobeamInstallException("Unexpected error during SoBeam installation!", e);
        } finally {
