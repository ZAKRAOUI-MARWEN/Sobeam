...
public class DefaultTbAlarmCommentService extends AbstractTbEntityService implements TbAlarmCommentService{
    ...
    @Override
    public AlarmComment saveAlarmComment(Alarm alarm, AlarmComment alarmComment, User user) throws SobeamException {
    ...
    @Override
    public void deleteAlarmComment(Alarm alarm, AlarmComment alarmComment, User user) throws SobeamException {
        if (alarmComment.getType() == AlarmCommentType.OTHER) {
            ...
            throw new SobeamException("System comment could not be deleted", SobeamErrorCode.BAD_REQUEST_PARAMS);
        }
    }
}
