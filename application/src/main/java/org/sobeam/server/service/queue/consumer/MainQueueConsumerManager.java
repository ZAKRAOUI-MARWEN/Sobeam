 ...
 */
package org.sobeam.server.service.queue.consumer;
...
import org.sobeam.server.service.queue.ruleengine.QueueEvent;
import org.sobeam.server.service.queue.ruleengine.TbQueueConsumerManagerTask;
import org.sobeam.server.service.queue.ruleengine.TbQueueConsumerTask;
...
public class MainQueueConsumerManager<M extends TbQueueMsg, C extends QueueConfig> {
    ...
    private void launchConsumer(TbQueueConsumerTask<M> consumerTask) {
        ...
        Future<?> consumerLoop = consumerExecutor.submit(() -> {
            SoBeamThreadFactory.updateCurrentThreadName(consumerTask.getKey().toString());
            try {
