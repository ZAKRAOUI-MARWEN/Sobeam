...
import org.sobeam.server.common.data.security.model.mfa.provider.TwoFaProviderConfig;
...
import org.sobeam.server.service.security.auth.mfa.provider.TwoFaProvider;
...
public class DefaultTwoFactorAuthService implements TwoFactorAuthService {
    ...
    private static final SobeamException ACCOUNT_NOT_CONFIGURED_ERROR = new SobeamException("2FA is not configured for account", SobeamErrorCode.BAD_REQUEST_PARAMS);
    private static final SobeamException PROVIDER_NOT_CONFIGURED_ERROR = new SobeamException("2FA provider is not configured", SobeamErrorCode.BAD_REQUEST_PARAMS);
    private static final SobeamException PROVIDER_NOT_AVAILABLE_ERROR = new SobeamException("2FA provider is not available", SobeamErrorCode.GENERAL);
    private static final SobeamException TOO_MANY_REQUESTS_ERROR = new SobeamException("Too many requests", SobeamErrorCode.TOO_MANY_REQUESTS);
    ...
    @Override
    public void checkProvider(TenantId tenantId, TwoFaProviderType providerType) throws SobeamException {
    ...
    @Override
    public void prepareVerificationCode(SecurityUser user, TwoFaAccountConfig accountConfig, boolean checkLimits) throws SobeamException {
    ...
    @Override
    public boolean checkVerificationCode(SecurityUser user, TwoFaProviderType providerType, String verificationCode, boolean checkLimits) throws SobeamException {
        TwoFaAccountConfig accountConfig = configManager.getTwoFaAccountConfig(user.getTenantId(), user.getId(), providerType)
                .orElseThrow(() -> ACCOUNT_NOT_CONFIGURED_ERROR);
    ...
    @Override
    public boolean checkVerificationCode(SecurityUser user, String verificationCode, TwoFaAccountConfig accountConfig, boolean checkLimits) throws SobeamException {
        if (!userService.findUserCredentialsByUserId(user.getTenantId(), user.getId()).isEnabled()) {
            throw new SobeamException("User is disabled", SobeamErrorCode.AUTHENTICATION);
        }
        ...
        if (checkLimits) {
            try {
                ...
                throw new SobeamException(e.getMessage(), SobeamErrorCode.AUTHENTICATION);
            }
    ...
    @Override
    public TwoFaAccountConfig generateNewAccountConfig(User user, TwoFaProviderType providerType) throws SobeamException {
    ...
    private TwoFaProviderConfig getTwoFaProviderConfig(TenantId tenantId, TwoFaProviderType providerType) throws SobeamException {
    ...
    private TwoFaProvider<TwoFaProviderConfig, TwoFaAccountConfig> getTwoFaProvider(TwoFaProviderType providerType) throws SobeamException {
