...
import org.sobeam.server.dao.util.DbTypeInfoComponent;
...
import org.sobeam.server.service.partition.AbstractPartitionBasedService;
...
public class DefaultDeviceStateService extends AbstractPartitionBasedService<DeviceId> implements DeviceStateService {
    ...
    public void init() {
        super.init();
        deviceStateExecutor = MoreExecutors.listeningDecorator(SoBeamExecutors.newWorkStealingPool(
                Math.max(4, Runtime.getRuntime().availableProcessors()), "device-state"));
        deviceStateCallbackExecutor = MoreExecutors.listeningDecorator(SoBeamExecutors.newWorkStealingPool(
