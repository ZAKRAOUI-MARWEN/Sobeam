 ...
 */
package org.sobeam.server.transport.lwm2m.server.ota;
...
import org.sobeam.server.cache.ota.OtaPackageDataCache;
...
import org.sobeam.server.common.data.ota.OtaPackageKey;
...
import org.sobeam.server.transport.lwm2m.server.ota.firmware.FirmwareDeliveryMethod;
import org.sobeam.server.transport.lwm2m.server.ota.firmware.FirmwareUpdateResult;
import org.sobeam.server.transport.lwm2m.server.ota.firmware.FirmwareUpdateState;
import org.sobeam.server.transport.lwm2m.server.ota.firmware.LwM2MClientFwOtaInfo;
import org.sobeam.server.transport.lwm2m.server.ota.firmware.LwM2MFirmwareUpdateStrategy;
import org.sobeam.server.transport.lwm2m.server.ota.software.LwM2MClientSwOtaInfo;
import org.sobeam.server.transport.lwm2m.server.ota.software.LwM2MSoftwareUpdateStrategy;
import org.sobeam.server.transport.lwm2m.server.ota.software.SoftwareUpdateResult;
import org.sobeam.server.transport.lwm2m.server.ota.software.SoftwareUpdateState;
import org.sobeam.server.transport.lwm2m.server.store.TbLwM2MClientOtaInfoStore;
...
import static org.sobeam.server.common.data.ota.OtaPackageKey.STATE;
...
import static org.sobeam.server.common.data.ota.OtaPackageUtil.getAttributeKey;
import static org.sobeam.server.transport.lwm2m.server.ota.firmware.FirmwareUpdateResult.UPDATE_SUCCESSFULLY;
import static org.sobeam.server.transport.lwm2m.server.ota.software.SoftwareUpdateResult.NOT_ENOUGH_STORAGE;
import static org.sobeam.server.transport.lwm2m.utils.LwM2MTransportUtil.LOG_LWM2M_TELEMETRY;
...
public class DefaultLwM2MOtaUpdateService extends LwM2MExecutorAwareService implements LwM2MOtaUpdateService {
    ...
    private void sendStateUpdateToTelemetry(LwM2mClient client, LwM2MClientOtaInfo<?, ?, ?> fwInfo, OtaPackageUpdateStatus status, String log) {
        ...
        result.add(kvProto.build());
        helper.sendParametersOnSobeamTelemetry(result, client.getSession(), client.getKeyTsLatestMap());
    }
