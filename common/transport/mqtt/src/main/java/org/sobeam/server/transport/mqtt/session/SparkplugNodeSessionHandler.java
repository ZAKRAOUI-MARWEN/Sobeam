...
import org.sobeam.server.common.adaptor.ProtoConverter;
...
import org.sobeam.server.transport.mqtt.MqttTransportHandler;
import org.sobeam.server.transport.mqtt.util.sparkplug.MetricDataType;
...
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugConnectionState.ONLINE;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugMessageType.DBIRTH;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugMessageType.NBIRTH;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugMetricUtil.createMetric;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugMetricUtil.fromSparkplugBMetricToKeyValueProto;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugMetricUtil.validatedValueByTypeMetric;
import static org.sobeam.server.transport.mqtt.util.sparkplug.SparkplugTopicUtil.parseTopicSubscribe;
...
public class SparkplugNodeSessionHandler extends AbstractGatewaySessionHandler<SparkplugDeviceSessionContext> {
    ...
    public void setNodeBirthMetrics(java.util.List<org.sobeam.server.gen.transport.mqtt.SparkplugBProto.Payload.Metric> metrics) {
    ...
    public void onAttributesTelemetryProto(int msgId, SparkplugBProto.Payload sparkplugBProto, SparkplugTopic topic) throws AdaptorException, SobeamException {
    ...
    public void handleSparkplugSubscribeMsg(List<Integer> grantedQoSList, MqttTopicSubscription subscription,
                                            MqttQoS reqQoS) throws SobeamException {
    ...
    private ListenableFuture<SparkplugDeviceSessionContext> onDeviceConnectProto(SparkplugTopic topic) throws SobeamException {
        try {
            ...
            throw new SobeamException(e, SobeamErrorCode.BAD_REQUEST_PARAMS);
        }
    }
    ...
    private List<TransportProtos.PostTelemetryMsg> convertToPostTelemetry(SparkplugBProto.Payload sparkplugBProto, Set<String> attributesMetricNames, String topicTypeName) throws AdaptorException {
        try {
            ...
            return msgs;
        } catch (IllegalStateException | JsonSyntaxException | SobeamException e) {
    ...
    private Optional<TransportProtos.PostAttributeMsg> getPostAttributeMsg(SparkplugBProto.Payload.Metric protoMetric) throws SobeamException {
    ...
    protected void sendErrorRpcResponse(TransportProtos.SessionInfoProto sessionInfo, int requestId, SobeamErrorCode result, String errorMsg) {
