...
public enum SparkplugMessageType {
	...
	public static SparkplugMessageType parseMessageType(String type) throws SobeamException {
		for (SparkplugMessageType messageType : SparkplugMessageType.values()) {
			if (messageType.name().equals(type)) {
				return messageType;
			}
		}
		throw new SobeamException("Invalid message type: " + type, SobeamErrorCode.INVALID_ARGUMENTS);
	}
