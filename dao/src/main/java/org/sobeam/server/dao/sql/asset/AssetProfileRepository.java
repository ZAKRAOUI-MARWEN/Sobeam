...
public interface AssetProfileRepository extends JpaRepository<AssetProfileEntity, UUID>, ExportableEntityRepository<AssetProfileEntity> {
    ...
    @Query("SELECT new org.sobeam.server.common.data.asset.AssetProfileInfo(a.id, a.tenantId, a.name, a.image, a.defaultDashboardId) " +
    ...
    @Query("SELECT new org.sobeam.server.common.data.EntityInfo(ap.id, 'ASSET_PROFILE', ap.name) " +
    ...
    @Query("SELECT new org.sobeam.server.common.data.EntityInfo(a.id, 'ASSET_PROFILE', a.name) " +
