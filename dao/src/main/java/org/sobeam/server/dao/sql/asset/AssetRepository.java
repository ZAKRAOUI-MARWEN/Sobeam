...
public interface AssetRepository extends JpaRepository<AssetEntity, UUID>, ExportableEntityRepository<AssetEntity> {
    ...
    @Query("SELECT new org.sobeam.server.dao.model.sql.AssetInfoEntity(a, c.title, c.additionalInfo, p.name) " +
            "FROM AssetEntity a " +
    ...
    @Query(value = "SELECT DISTINCT new org.sobeam.server.common.data.util.TbPair(a.tenantId , a.type) FROM  AssetEntity a")
