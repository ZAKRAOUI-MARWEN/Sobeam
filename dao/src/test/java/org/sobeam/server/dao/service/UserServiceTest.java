...
public class UserServiceTest extends AbstractServiceTest {
    ...
    public void before() {
        ...
        tenantAdmin.setEmail("tenant@sobeam.org");
        ...
        customerUser.setEmail("customer@sobeam.org");
    ...
    public void testFindUserByEmail() {
        User user = userService.findUserByEmail(SYSTEM_TENANT_ID, "sysadmin@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "tenant@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "customer@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "fake@sobeam.org");
        Assert.assertNull(user);
    }

    @Test
    public void testFindUserByTenantIdAndEmail() {
        User user = userService.findUserByTenantIdAndEmail(SYSTEM_TENANT_ID, "sysadmin@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "tenant@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "customer@sobeam.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "fake@sobeam.org");
        Assert.assertNull(user);
    }
    ...
    public void testSaveUser() {
        User tenantAdminUser = userService.findUserByEmail(SYSTEM_TENANT_ID,"tenant@sobeam.org");
        User user = new User();
    ...
    public void testSaveUserWithSameEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@sobeam.org");
        tenantAdminUser.setEmail("sysadmin@sobeam.org");
        Assertions.assertThatThrownBy(() -> userService.saveUser(tenantId, tenantAdminUser))
                .isInstanceOf(DataValidationException.class)
                .hasMessage("User with email 'sysadmin@sobeam.org' already present in database!");
    }

    @Test
    public void testSaveUserWithInvalidEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@sobeam.org");
        tenantAdminUser.setEmail("tenant_sobeam.org");
        Assertions.assertThatThrownBy(() -> userService.saveUser(tenantId, tenantAdminUser))
                .isInstanceOf(DataValidationException.class)
                .hasMessage("Invalid email address format 'tenant_sobeam.org'!");
    }
    ...
    public void testFindCustomerUsers() {
        User customerUser = userService.findUserByEmail(tenantId, "customer@sobeam.org");
