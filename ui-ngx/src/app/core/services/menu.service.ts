...
export class MenuService {
  ...
  private updateOpenedMenuSections() {
    ...
    this.currentMenuSections.filter(section => section.type === 'toggle' &&
      (url.startsWith(section.path) || openedMenuSections.includes(section.path))).forEach(
        section => {
          section.opened = section.path === '/dashboards' ? false : true;
      }
    );
  }
  ...
  refreshTenantDashboards() {
    this.dashboardService.getTenantDashboards(new PageLink(10, 0, null, { property: 'createdTime', direction: Direction.ASC })
    ).subscribe(res => {
      this.tenantDashboards = res?.data || [];
      this.tenantDashboards.forEach(dashboard => {
        this.pages.push({
          id: dashboard.id,
          name: dashboard.name || 'Untitled',
          type: 'link',
          path: `/dashboards/${dashboard.id.id}`,
          icon: dashboard.icon || 'default-icon'
        });
      });
    });
  }
