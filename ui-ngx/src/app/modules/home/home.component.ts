...
export class HomeComponent extends PageComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.icon = savedMode  ? savedMode : 'sunny';
    if (this.icon === 'sunny') {
      this.renderer.removeClass(this.document.body, 'tb-dark');
      this.renderer.addClass(this.document.body, 'tb-default');
       } else {
      this.renderer.removeClass(this.document.body, 'tb-default');
      this.renderer.addClass(this.document.body, 'tb-dark');
    }
  ...
  changeMode() {
    ...
    localStorage.setItem('mode', this.icon);
  }
